...
Both `--exclude` and `--include` are **repeatable** — specify each pattern as a
separate flag. All exclude patterns are OR-combined, and all include patterns are
OR-combined:

```bash
# Multiple --exclude and --include flags
./replica-sync/scripts/build-exclude-list.sh \
  --exclude ".github/workflows/**" \
  --exclude "services/internal-*/**" \
  --exclude "docs/INTERNAL_*.md" \
  --include ".github/workflows/sync-to-wiki-main.yml" \
  --include ".github/workflows/sync-replica.yml"
```

The generated output records all patterns in a comment block for traceability:

```bash
# Generated by build-exclude-list.sh
# exclude: .github/workflows/**
# exclude: services/internal-*/**
# exclude: docs/INTERNAL_*.md
# include: .github/workflows/sync-to-wiki-main.yml
...
EXCLUDE_PATHS=(
  ...
  "services/internal-auth/main.go"
